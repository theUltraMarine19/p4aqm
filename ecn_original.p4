...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t standard_metadata) { 
	...
	apply {
		...
		if (standard_metadata.instance_type == BMV2_V1MODEL_INSTANCE_TYPE_EGRESS_CLONE) {
			...
			if (3 * standard_metadata.enq_qdepth <= (10 * (bit<19>)meta.total) && standard_metadata.deq_qdepth >= ECN_THRESHOLD) { // threshold on egress queue since egress proessing is more comp. intensive
